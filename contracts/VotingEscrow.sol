...
/// source: https://github.com/fiatdao/veToken
...
contract VotingEscrow is IVotingEscrow, IVotes, ReentrancyGuard {
    ...
    uint256 public constant MAXTIME = 1460 days;
    ...
    function forceUndelegate(address _addr) external override {
        ...
        if (delegatee != _addr && value > 0) {
            ...
            emit DelegateChanged(msg.sender, delegatee, _addr);
        }
    }
    ...
    function delegate(
        address _addr
    ) external override(IVotingEscrow, IVotes) nonReentrant checkBlocklist {
     ...
     */
    function getVotes(address _addr) external view override returns (uint256) {
        return balanceOf(_addr);
    }

    /**
     * @dev Same as balanceOfAt
     */
    ...
    /**
     * @dev Returns the delegate that `_ddr` has chosen.
     */
    function delegates(address _addr) external view override returns (address) {
        return locked[_addr].delegatee;
    }
