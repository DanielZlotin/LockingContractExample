...
import { locking, xctd, rewardToken, user1, user2, withFixture, fundWithXCTD, deployer, advanceMonths, INITIAL_MONTH, rewards } from "./fixture";
...
describe("Rewards", () => {
  ...
  describe("with tokens", () => {
    ...
    beforeEach(async () => {
      await rewardToken.methods.approve(rewards.options.address, await rewardToken.amount(50_000)).send({ from: deployer });
      // 50000 / (60 * 86400) = 0.00964506
      await rewards.methods.addReward(rewardToken.options.address, 0, 5, (await rewardToken.amount(10_000)).toFixed(0)).send({ from: deployer });
    });
    ...
    it("user should have reward if did stake and didn't claim so far", async () => {
      ...
      const pendingRewards = await rewards.methods.pendingRewards(user1, rewardToken.options.address).call();
    ...
    it("two users, same balance, same period", async () => {
      ...
      const userOnePendingRewards = await rewards.methods.pendingRewards(user1, rewardToken.options.address).call();
      const userTwoPendingRewards = await rewards.methods.pendingRewards(user2, rewardToken.options.address).call();
    ...
    it("one user locks one month after reward was added, reward program ended, rewards are orphaned", async () => {
      ...
      const rewardTokenBalance = BN(await rewardToken.methods.balanceOf(rewards.options.address).call());
      expect(rewardTokenBalance.minus(pendingRewards)).bignumber.closeTo(bn18(20_000), 1e18);

      await rewards.methods.claim(user1, rewardToken.options.address).send({ from: user1 });
      expect(await rewardToken.methods.balanceOf(rewards.options.address).call()).bignumber.closeTo(bn18(20_000), 1e18);
    });
    ...
    it("two users lock different periods, claim at the end of the program", async () => {
      ...
      await rewards.methods.claim(user2, rewardToken.options.address).send({ from: user2 });
    ...
    it("rewards program is updateable, same period", async () => {
      await rewardToken.methods.approve(rewards.options.address, await rewardToken.amount(15_000)).send({ from: deployer });
      await rewards.methods.addReward(rewardToken.options.address, 0, 5, (await rewardToken.amount(3_000)).toFixed(0)).send({ from: deployer });
    ...
    it("rewards program is updateable, offset", async () => {
      await rewardToken.methods.approve(rewards.options.address, await rewardToken.amount(15_000)).send({ from: deployer });
      await rewards.methods.addReward(rewardToken.options.address, 10, 5, (await rewardToken.amount(3_000)).toFixed(0)).send({ from: deployer });
    ...
    it("owner can only recover reward funds per month a single time", async () => {
      const initialBalance = await rewardToken.methods.balanceOf(deployer).call();
      await advanceMonths(6);
      await rewards.methods.recover(rewardToken.options.address, INITIAL_MONTH, INITIAL_MONTH + 5).send({ from: deployer });
    ...
    it("recover reward tokens that don't belong to the reward program", async () => {
      await locking.methods.lock(await xctd.amount(amount), 24).send({ from: user1 });
      await rewardToken.methods.transfer(rewards.options.address, await rewardToken.amount(17_000)).send({ from: deployer });
      const balanceBefore = await rewardToken.methods.balanceOf(deployer).call();
      await advanceMonths(1);
      await rewards.methods.recover(rewardToken.options.address, 0, 0).send({ from: deployer });
